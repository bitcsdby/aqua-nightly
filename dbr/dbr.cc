...
int DBR_Agent::pkt_saved = 0;
int DBR_Agent::pkt_all = 0;
...
void DBR_Agent::recv(Packet *p, Handler *)
{
	...
	//printf("No %d get the broadcast pkt\n",this->addr());
	...
	//I am source and sending the packet
	if ((src == mn_->address()) &&
	    (cmh->num_forwards() == 0))
	{// packet I'm originating
		...
		// from uwsink.cc sendpkt->classifier.send->cmu.send->dbr.send
	...
	//delay stratagy  need to be understood
	if ((src == mn_->address()) &&
	    (dbrh->mode() == DBRH_DATA_GREEDY))
