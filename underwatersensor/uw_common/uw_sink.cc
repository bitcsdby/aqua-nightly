...
//#ifdef DBR_PRO
	//dby change
	    ...
	    iph->src_=here_;
	    iph->dst_.addr_= target_id.addr_;
	    iph->dst_.port_=255;
	...
	//      printf("uw_sink:source(%d,%d) send packet %d at %lf : the coordinates of target is (%lf,%lf,%lf) and range=%lf and my position (%f,%f,%f) and cx is(%f,%f,%f)  type is %d\n", dbh->sender_id.addr_,vbh->sender_id.port_,vbh->pk_num, NOW, vbh->info.tx=target_x,vbh->info.ty=target_y, vbh->info.tz=target_z,vbh->range,node->X(),node->Y(),node->Z(),node->CX(),node->CY(),node->CZ(),vbh->mess_type);
	      printf("uw_sink:source(%d) send packet %d at %lf : the coordinates of target is (%lf,%lf,%lf) and range=%lf and my position (%f,%f,%f) and cx is(%f,%f,%f)  \n", dbh->prev_hop(),pk_count, NOW, target_x,target_y, target_z,range_,node->X(),node->Y(),node->Z(),node->CX(),node->CY(),node->CZ());
...
*/
	 ...
	 */
	 ...
	 printf("UWSink (id:%d): I get the packet data no.%d . Source is %d Prehop is %d \n",here_.addr_,dbh->packetID(),dbh->owner(),dbh->prev_hop());
	 cum_delay = cum_delay + (NOW - dbh->ts_);
	 num_recv++;
