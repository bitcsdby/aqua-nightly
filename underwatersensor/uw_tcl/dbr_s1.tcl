...
set opt(nn) 5 ;# number of nodes  surface and underwater
...
set opt(range) 800 ;#range of each node in meters
...
#$a_(2) cmd set-target-x 50
#$a_(2) cmd set-target-y 50
#$a_(2) cmd set-target-z 100
$a_(2) cmd set-filename $opt(datafile)
$a_(2) cmd set-packetsize $opt(packet_size) ;# # of bytes
$a_(2) set data_rate_ [expr 1.0/10]
#$a_(2) set data_rate_ [expr 1.0/30]

#Set the Sink node 4  do
...
$a_(3) set data_rate_ [expr 1.0/10]
#$a_(2) set data_rate_ [expr 1.0/30]
# sink set end
...
#$ns_ at 0.0  "$node_(4) move"
$ns_ at 2.0 "$a_(0) cbr-start"
$ns_ at 5.0 "$a_(1) cbr-start"
$ns_ at 7.0 "$a_(2) cbr-start"
$ns_ at 9.0 "$a_(3) cbr-start"
$ns_ at $opt(stop).001 "$a_(0) terminate"
$ns_ at $opt(stop).002 "$a_(1) terminate"
$ns_ at $opt(stop).003 "$a_(2) terminate"
$ns_ at $opt(stop).004 "$a_(3) terminate"
$ns_ at $opt(stop).004 "$a_(4) terminate"
$ns_ at $opt(stop).005 "$god_ compute_energy"
$ns_ at $opt(stop).006 "$ns_ nam-end-wireless $opt(stop)"


$ns_ at $opt(stop).007 "$god_ compute_energy"
$ns_ at $opt(stop).008 "$ns_ nam-end-wireless $opt(stop)"
$ns_ at $opt(stop).009 "puts \"NS EXISTING...\"; $ns_ halt"
